<template>
	<view class="bodyMain bodyBG hideScroll" style="overflow: auto;">
		...
		<u-row style="margin-top: 40rpx;">
			<u-col>
				<u-search :placeholder="getCityDisplay(searchCities)" actionText="取消" @custom="goHome" disabled></u-search>
			</u-col>
		</u-row>
		...
		<u-row style="margin-top: 20rpx;">
			...
			<u-col span="4" textAlign="center">
				<view class="tab" @click="changeTabCurrent(1)">
					<text class="text font-PingFangSCBold">{{ timeCheckedModel.name }}</text>
				</view>
			</u-col>
			<u-col span="4" textAlign="center">
		...
		<!-- 精确筛选 -->
		<u-row style="margin-top: 10rpx;" justify="center">
			<u-col span="4" textAlign="center">
				<u-button @click="exactSearch()" icon="search" color="rgb(241, 89, 142)" shape="circle">精确筛选</u-button>
			</u-col>
		</u-row>
<!-- 		<view class="resetFilters flex-center">
			<view class="btn flex-center padding" @click="exactSearch()">
				<text class="uni-icon">&#xe60c;</text>
				<text class="text font-PingFangSCMedium">精确筛选</text>
			</view>
		</view> -->
		<!-- 按列表显示 -->
		<view v-if="isList" class="main">
			<z-paging :default-page-size="6" :auto-show-back-to-top="true" ref="paging" v-model="listStaffs"
				@query="queryList">
				<template v-for="(item, index) in listStaffs">
					<cell :key="item.id">
						...
						<view v-else class="itemList">
							<view class="picture" @click.stop="navigateTo(item.employeeId)">
								...
								<view class="tagBox flex-space-between">
									<view class="left">
										<text
											class="flex-center font-PingFangSCMedium text">{{ getClassName(item.classificationId) }}</text>
										<text
											class="flex-center font-PingFangSCMedium text">${{ item.lowestPrice || 0 }}/{{ (item.lowestPriceMinute || 0) / 60 }}h</text>
									</view>
									<text
										class="flex-center font-PingFangSCMedium text pink">{{ item.status === 1 ? "我有时间" : "等我有空" }}</text>
								</view>
							</view>
							...
							<view class="info">
								<view class="flex-space-between details">
									...
									<text v-if="item.isFocusOn" class="uni-icon active heart"
										@click.stop="followOperate(item)">&#xe60f;</text>
									<text v-else class="uni-icon heart"
										@click.stop="followOperate(item)">&#xe60e;</text>
								</view>
								<view class="flex-start details">
									<text class="font-PingFangSCMedium text">{{ item.city || 'AUS' }},
										{{ item.province || 'AUS' }}{{ getPostCode(item.street) }}</text>
								</view>
								<view class="flex-start details">
		...
		<view v-else class="flex-end">
			<web-view ref="mapWebView" class="webView" :style="`height: ${SystemInfo.screenHeight - 190}px;`"
				:src="WebViewUrl" @onPostMessage="handlePostMessage"></web-view>
		</view>
		<!-- 排序 -->
		<u-action-sheet cancel-text="取消" v-model="popupOrder" @click="sortChecked" :list="sortList" :safe-area-inset-bottom="true"/>
		<u-action-sheet cancel-text="取消" v-model="popupTime" @click="queryByDate(item)" :list="timeList" :safe-area-inset-bottom="true"/>
		<!-- <uni-popup ref="popup" type="bottom" mask-click :safe-area="false"> -->
			<!-- <view class="popupBox" v-if="popupType === 'slot'">
				...
				<view class="list flex-center" v-for="(item, index) in sortList" :key="index"
					@click="sortChecked(index)">
					<text class="uni-icon checked text" v-if="item.checked">&#xe60d;</text>
					<text class="font-PingFangSCMedium text">{{ item.name }}</text>
				</view>
				<view :class="['cancel flex-center', { isAndroid: !isAndroid }]" @click="cancelPopup"><text
						class="font-PingFangSCMedium text">取消</text></view>
			</view> -->
			<!-- 时间 -->
<!-- 			<view class="popupBox timeBox" v-else>
				...
				<scroll-view scroll-y class="scrollBox"
					:style="`height: ${SystemInfo.safeArea.height - SystemInfo.safeArea.top - (isAndroid ? 34 : 0)}px;`">
					<view class="timeList flex-space-between" v-for="(item, index) in timeList" :key="item.id"
						@click="queryByDate(item)">
						<text class="text flex-center font-PingFangSCMedium">{{ item.name }}</text>
						<text v-if="item.num > 0"
							class="text flex-center font-PingFangSCMedium count pink">{{ item.num }}</text>
...
<script>
	...
	import {
		queryEmployeeList,
		queryByOrderConformity,
		followStaff,
		unFollowStaff
	} from "@/common/js/apiCommon.js";
	import {
		noLocationPrompt
	...
	export default {
		...
		methods: {
			...
			sortChecked(index) {
				...
				if (!cur.checked) {
					cur.checked = true;
					this.sortList.filter(item => item.checked && item.id !== cur.id).map(item => {
						item.checked = false;
					});
					if (this.isList) {
			...
			getInfoByLocation() {
				...
				this.WebViewUrl =
					`/hybrid/html/maps.html?key=${$base.googleMapAPIKey}&address=${encodeURIComponent(JSON.stringify(this.currentLocation))}`;
